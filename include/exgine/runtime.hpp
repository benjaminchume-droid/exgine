...
bool generate_building(EntityId,BuildingConfig={});bool set_building_door(EntityId,std::uint64_t,bool)noexcept;const BuildingInstance*building(EntityId)const noexcept;std::vector<BuildingCollisionVolume>building_collision(EntityId)const;const BuildingRoom*find_building_room(EntityId,Vec3)const noexcept;
