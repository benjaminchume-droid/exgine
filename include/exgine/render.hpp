...
struct RenderConfig{RenderBackend backend=RenderBackend::Headless;std::uint32_t width=1280,height=720;bool frustum_culling=true;bool high_dynamic_range=true;std::uint32_t max_lights=256;};
...
struct RenderStats{std::size_t submitted_draws=0,visible_draws=0,culled_draws=0,submitted_lights=0;};
