...
// Phase 54: explicit frame graph ordering and dependency validation.
...
class RenderFeatureGraph {
public:
    bool add(RenderFeatureNode node);
    bool set_enabled(std::string_view name,bool enabled) noexcept;
    [[nodiscard]] RenderExecutionPlan compile() const;
    void clear() noexcept { nodes_.clear(); }
    [[nodiscard]] std::size_t size() const noexcept { return nodes_.size(); }
private:
    std::unordered_map<std::string,RenderFeatureNode> nodes_;
};

// Phase 55: temporal reconstruction/history state used by TAA/upscaling/reflections.
...
public:
    ...
    void reset() noexcept { history_.valid=false; history_.frame_id=0; history_.jitter={}; }
    [[nodiscard]] TemporalJitter next_jitter() noexcept;
    void begin_frame(std::uint64_t frame_id,float motion_magnitude,float disocclusion) noexcept;
    [[nodiscard]] const TemporalHistory& history() const noexcept { return history_; }
private:
    TemporalHistory history_{};
};

// Phase 56: complete game-flow lifecycle independent of any genre.
...
public:
    bool transition(GameFlowState next,std::string error={});
    [[nodiscard]] bool can_transition(GameFlowState next) const noexcept;
    [[nodiscard]] GameFlowState state() const noexcept { return snapshot_.state; }
    [[nodiscard]] const GameFlowSnapshot& snapshot() const noexcept { return snapshot_; }
};

// Phase 57: lightweight engine telemetry with deterministic aggregate timing.
...
public:
    ...
    void end(std::string_view name,double milliseconds) noexcept;
    void add_counter(std::string_view name,std::uint64_t amount=1) noexcept;
    [[nodiscard]] const ProfileSample* sample(std::string_view name) const noexcept;
...
private:
    ...
    std::unordered_map<std::string,std::uint64_t> active_;
};

// Phase 58: bounded rollback history for deterministic/networked simulation.
...
public:
    ...
    bool push(std::uint32_t tick,std::vector<std::uint8_t> state);
    [[nodiscard]] const RollbackFrame* exact(std::uint32_t tick) const noexcept;
    [[nodiscard]] const RollbackFrame* latest_at_or_before(std::uint32_t tick) const noexcept;
    bool discard_after(std::uint32_t tick) noexcept;
    void clear() noexcept { frames_.clear(); }
    [[nodiscard]] std::size_t size() const noexcept { return frames_.size(); }
private:
    std::size_t capacity_=120;
    std::vector<RollbackFrame> frames_;
};

// Phase 59: camera/exposure/presentation state shared by HDR and environment rendering.
struct PresentationState { float exposure=0,contrast=1,white_point=1,sharpening=0; bool hdr=true; };
class WorldPresentation {
public:
    void set_sun_elevation(float radians) noexcept;
    void set_weather_fog(float density) noexcept;
    void set_auto_exposure(float luminance,double dt) noexcept;
    [[nodiscard]] const PresentationState& state() const noexcept { return state_; }
private:
    PresentationState state_{};
};

// Phase 60: repository-independent production acceptance gate.
