...
public:
    ...
    [[nodiscard]] bool submit(const SoundSample&) noexcept;
    void clear() noexcept;
    [[nodiscard]] bool running() const noexcept{return running_;}
private:
    static constexpr std::size_t ring_samples=262144;
    void* stream_=nullptr;
    std::array<float,ring_samples> ring_{};
    std::atomic<std::size_t> read_{0},write_{0};
    std::uint32_t sample_rate_=48000,channels_=2;
    std::atomic<float> gain_{1},pan_{0};
    bool running_=false;
#if defined(__ANDROID__)
    static int32_t audio_callback(void*,void*,void*,int32_t) noexcept;
#endif
};
} // namespace exgine
