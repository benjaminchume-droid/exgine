...
int main(int argc, char** argv) {
    ...
    auto loader = [root](std::string_view uri, std::string& out) {
        out = read_text(root / std::string(uri));
        return !out.empty();
    };
    const auto manifest = read_text(root / "project.exg");
    if (manifest.empty()) { std::cerr << "showcase: project.exg not found\n"; return 1; }

    exgine::ShowcaseGame game(loader);
    if (!game.open(manifest) || !game.start()) { std::cerr << "showcase: startup failed\n"; return 2; }

    exgine::RenderFrame frame;
    exgine::RenderResult render;
    constexpr int sample_frames = 300;
    for (int i = 0; i < sample_frames; ++i) {
        if (!game.update(1.0 / 60.0) || !game.build_frame(frame, render)) {
            std::cerr << "showcase: frame " << i << " failed\n";
            return 3;
        }
    }

    const auto saved = game.save();
    if (saved.empty() || !game.restore(saved)) { std::cerr << "showcase: save round-trip failed\n"; return 4; }
    const auto& m = game.metrics();
    std::cout << "EXGINE SHOWCASE\n"
