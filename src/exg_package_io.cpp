...
bool open_exg_package_asset(::AAssetManager*m,const char*name,ExgPackage&o,std::string&e)noexcept{if(!m||!name){e="invalid Android EXG asset request";return false;}AAsset*a=AAssetManager_open(m,name,AASSET_MODE_BUFFER);if(!a){e="unable to open EXG asset";return false;}const auto*n=AAsset_getBuffer(a);auto sz=AAsset_getLength64(a);if(!n||sz<=0){AAsset_close(a);e="EXG asset is empty";return false;}std::vector<std::byte>b(static_cast<std::size_t>(sz));std::memcpy(b.data(),n,b.size());AAsset_close(a);return open_exg_package(b,o,e);}
#endif
}