...
bool parse_ref(const std::string& token, Key& out) {
    ...
    if (slot == 0 || vals[0] == 0) return false;
    out={vals[0],vals[1],vals[2]}; return true;
}
...
ImportResult import_obj(std::string_view uri,std::string_view text) {
    ...
    while (std::getline(in,line)) {
        ...
        std::istringstream ls(line); std::string op; ls>>op; if(op.empty()||op[0]=='#') continue;
    ...
    ImportedMesh m; m.uri=std::string{uri}; m.mesh=std::move(mesh); m.asset_id=make_asset_id(uri,std::vector<std::uint8_t>(text.begin(),text.end())); r.meshes.push_back(std::move(m)); r.success=true; return r;
}
