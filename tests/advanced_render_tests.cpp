...
int main(){
    ...
    AtmosphereSettings atmosphere; auto sky=evaluate_sky(atmosphere,{0,1,0},{0,1,0}); assert(sky.r>=0&&sky.g>=0&&sky.b>=0);
    ...
    auto water=sample_water(WaterSettings{}, {1,0,2}, 3); assert(std::isfinite(water.height)); assert(std::fabs(std::sqrt(water.normal.x*water.normal.x+water.normal.y*water.normal.y+water.normal.z*water.normal.z)-1.0f)<0.01f);
