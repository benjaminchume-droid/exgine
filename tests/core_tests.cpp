...
void test_world_determinism(){const exgine::TerrainConfig c{20,1337,32.0f};const exgine::ProceduralWorld a(c),b(c);assert(std::isfinite(a.sample_height(32,64))&&a.sample_height(32,64)==b.sample_height(32,64));}
...
void test_runtime(){const auto c=exgine::Compiler{}.compile(valid_source);assert(c.succeeded());exgine::Runtime r;assert(r.load(*c.ir));assert(r.state().entities.size()==4);const auto id=r.state().world_entity+1;assert(r.attach_geometry(id,exgine::MeshAssembly{{{"house_mesh",exgine::make_box({{8,3,8}}),"concrete",{},{1,1,1}}}}));r.update(.5);r.update(.25);assert(r.state().tick==2&&r.state().elapsed_seconds==.75);r.reset();assert(r.state().entities.size()==0);}
...
void test_textures(){exgine::TextureGenerationSettings s;s.width=32;s.height=24;s.noise.seed=123;s.noise.type=exgine::NoiseType::Fbm;auto t=exgine::generate_noise_texture(s,1);assert(t.valid()&&t.data.size()==32u*24u);assert(exgine::sample_noise(.2f,.7f,s.noise)==exgine::sample_noise(.2f,.7f,s.noise));auto m=exgine::make_real_world_material("wood",7);auto set=exgine::generate_material_textures(m,s);assert(set.valid()&&set.base_color->width==32&&set.base_color->channels==3);}
...
int main(){test_version();test_diagnostics();test_ir();test_world_determinism();test_language_pipeline();test_runtime();test_geometry();test_characters();test_materials();test_textures();test_resources_and_runtime_materials();test_engine();return 0;}
